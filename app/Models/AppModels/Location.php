...
class Location extends Model
{
	...
	public static function updateshippingaddress($request)
	{
			...
			if($authenticate==1){
				...
				if($is_default == '1'){
					...
					DB::table('customers')->where('user_id', $user_id)->where('address_book_id',$address_book_id)->update(['is_default' => $is_default]);
						}
				}
	...
	if($authenticate==1){
		...
		DB::table('user_to_address')->where('user_id', $user_id)->where('address_book_id',$address_book_id)
		->update(['is_default' => 1]);
