...
class Payments_setting extends Model
{
    ...
    public function updaterecord($request){
      ...
      $result = DB::table('payment_methods_detail')->where('payment_methods_detail.payment_methods_id',$request->id)->get();
       ...
       foreach($languages as $languages_data){
   			...
   			if($checkExist){
   				DB::table('payment_description')->where('payment_methods_id','=',$id)->where('language_id','=',$languages_data->languages_id)->update([
   					'name'  	    		 =>   $request->$name,
   					'language_id'			 =>   $languages_data->languages_id,
   					]);
   			}else{
    ...
    public function display($id){
      $result = array();
      $result['payment_methods'] = Payments_setting::first();
      $setting = new Setting();
      $myVarsetting = new SiteSettingController($setting);
      $languages = $myVarsetting->getLanguages();
