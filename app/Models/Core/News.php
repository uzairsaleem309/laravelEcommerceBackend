    ...
    public function filter($request){
      ...
      if($name=="Name"){
        $language_id = '1';
          ...
          $news = NewsToNewsCategory::sortable()
              ->leftJoin('news_categories', 'news_categories.categories_id', '=', 'news_to_news_categories.categories_id')
              ->leftJoin('news', 'news.news_id', '=', 'news_to_news_categories.news_id')
              ->leftJoin('news_description','news_description.news_id','=','news.news_id')
              ->leftJoin('news_categories_description','news_categories_description.categories_id','=','news_to_news_categories.categories_id')
              ...
              ->leftJoin('image_categories','image_categories.image_id', '=', 'news.news_image')
              ->select('news_to_news_categories.*', 'news_categories_description.categories_name','news_categories.*', 'news.*', 'news.created_at as date_added','news.updated_at as date_updated','news_description.news_name','news_description.*','image_categories.path as path')
              ->where('news_description.language_id','=', $language_id)
              ->where('news_categories_description.language_id','=', $language_id)
              ->where('image_categories.image_type','=','THUMBNAIL' )->orWhere('image_categories.image_type','=','ACTUAL')
              ->where('news_description.news_name', 'LIKE', '%' . $param . '%')->orderBy('news.news_id', 'ASC')
              ->paginate(20);
          ...
          $news = NewsToNewsCategory::sortable()
              ...
              ->leftJoin('images','images.id', '=', 'news.news_image')
              ->leftJoin('image_categories','image_categories.image_id', '=', 'news.news_image')
              ->select('news_to_news_categories.*', 'news_categories_description.categories_name','news_categories.*', 'news.*', 'news.created_at as date_added','news.updated_at as date_updated','news_description.*','image_categories.path as path')
              ->where('news_description.language_id','=', $language_id)
              ->where('news_categories_description.language_id','=', $language_id)
             ->where('image_categories.image_type','=','THUMBNAIL' )->orWhere('image_categories.image_type','=','ACTUAL')
              ->orderBy('news.news_id', 'ASC')
